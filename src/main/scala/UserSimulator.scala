...
/*        response.onComplete {
      ...
      case UpdateAlbum(id) =>
        ...
        var A = new Album(id, 12, "cover_photo", "time", "description: String",
...
/*        	print("user_comments = ")
  ...
  case CreateStatus =>
    ...
    import FBJsonProtocol._
    var S = new Status("null", "now", self.path.name, "location", status,
                  "time again", "-1")
  ...
  case UpdateStatus(id) =>
    ...
    import FBJsonProtocol._
    var S = new Status(id, "now", self.path.name, "location", status,
                  "time again", "-1")
    val response: Future[HttpResponse] = pipeline(Post("http://localhost:8080/Status", S))
